...
class CollectionAPITest(TestCase):
    ...
    def test__ensure_partial_filter_expression_unique_index(self):
        self.db.collection.create_index(fields=[('partialFilterExpression_value', 1), ('value', 1)], unique=True, partialFilterExpression={"partialFilterExpression_value":{"$eq":1}})

        # we should be able to add documents with duplicated `value` and `partialFilterExpression_value` if `partialFilterExpression_value` isn't set to 1
        self.db.collection.insert({'partialFilterExpression_value': 3, 'value':4})
        self.db.collection.insert({'partialFilterExpression_value': 3, 'value':4})

        # we should be able to add documents with distinct `value` values and duplicated `partialFilterExpression_value` value set to 1.
        self.db.collection.insert({'partialFilterExpression_value': 1, 'value':2})
        self.db.collection.insert({'partialFilterExpression_value': 1, 'value':3})

        # we should not be able to add documents with duplicated `partialFilterExpression_value` and `value` values if `partialFilterExpression_value` is 1.
        with self.assertRaises(mongomock.DuplicateKeyError):
            self.db.collection.insert({'partialFilterExpression_value': 1, 'value': 3})

...
class MongoClientCollectionTest(_CollectionComparisonTest):
    ...
    def test__estimated_document_count(self):
        ...
        self.cmp.do.insert_one({'a': 0})
        self.cmp.compare.estimated_document_count()
        if version.parse('5') > SERVER_VERSION:
    ...
    def test__find_by_objectid_in_list(self):
        # See #79
        self.cmp.do.insert_one({'_id': 'x', 'rel_id': [ObjectId('52d669dcad547f059424f783')]})
    ...
    def test__find_not_exceptions(self):
        self.cmp.do.insert_one({'noise': 'longhorn'})
        with self.assertRaises(OperationFailure):
    ...
    def test__find_compare(self):
        self.cmp.do.insert_one({'noise': 'longhorn', 'sqrd': 'non numeric'})
        for x in range(10):
            self.cmp.do.insert_one({'num': x, 'sqrd': x * x})
    ...
    def test__find_compare_nested_objects(self):
        ...
        self.cmp.compare_ignore_order.find({'counts': {'$gt': {'circles': {'blue': 1}}}})

    def test__find_sets(self):
        single = 4
        even = [2, 4, 6, 8]
        prime = [2, 3, 5, 7]
        self.cmp.do.insert_many([{'x': single}, {'x': even}, {'x': prime}, {}])
    ...
    def test__find_operators_in_list(self):
        self.cmp.do.insert_many([{'x': 4}, {'x': [300, 500, 4]}, {'x': [1200, 300, 1400]}])
    ...
    def test__find_one_and_replace(self):
        self.cmp.do.insert_many([{'a': i} for i in range(10)])
        self.cmp.compare.find_one_and_replace({'a': 5}, {'a': 11}, projection={'_id': False})
        self.cmp.compare.find()
    ...
    def test__return_only_selected_fields_no_id(self):
        self.cmp.do.insert_one({'name': 'Chucky', 'type': 'doll', 'model': 'v6'})
        self.cmp.compare_ignore_order.find({'name': 'Chucky'}, projection={'type': 1, '_id': 0})

    def test__return_only_selected_fields_nested_field_found(self):
        self.cmp.do.insert_one({'name': 'Chucky', 'properties': {'type': 'doll', 'model': 'v6'}})
        self.cmp.compare_ignore_order.find({'name': 'Chucky'}, projection=['properties.type'])

    def test__return_only_selected_fields_nested_field_not_found(self):
        self.cmp.do.insert_one({'name': 'Chucky', 'properties': {'type': 'doll', 'model': 'v6'}})
        self.cmp.compare_ignore_order.find({'name': 'Chucky'}, projection=['properties.color'])
    ...
    def test__exclude_selected_fields_including_id(self):
        self.cmp.do.insert_one({'name': 'Chucky', 'type': 'doll', 'model': 'v6'})
        self.cmp.compare_ignore_order.find({'name': 'Chucky'}, projection={'type': 0, '_id': 0})

    def test__exclude_all_fields_including_id(self):
        self.cmp.do.insert_one({'name': 'Chucky', 'type': 'doll'})
        self.cmp.compare.find({'name': 'Chucky'}, projection={'type': 0, '_id': 0, 'name': 0})

    def test__exclude_selected_nested_fields(self):
        self.cmp.do.insert_one({'name': 'Chucky', 'properties': {'type': 'doll', 'model': 'v6'}})
        self.cmp.compare_ignore_order.find({'name': 'Chucky'}, projection={'properties.type': 0})
    ...
    def test__set_subdocuments_positional(self):
        self.cmp.do.insert_one(
            {'name': 'bob', 'subdocs': [{'id': 1, 'name': 'foo'}, {'id': 2, 'name': 'bar'}]}
        )
        self.cmp.do.update_many(
            {'name': 'bob', 'subdocs.id': 2}, {'$set': {'subdocs.$': {'id': 3, 'name': 'baz'}}}
        )
        self.cmp.compare.find()
    ...
    def test__inc_subdocument_positional(self):
        ...
        self.cmp.do.update_many(
            {'name': 'bob', 'data': {'$elemMatch': {'age': 0}}}, {'$inc': {'data.$.age': 1}}
        )
        self.cmp.compare.find()
    ...
    def test__setOnInsert_subdocument_upsert(self):
        self.cmp.do.delete_many({})
        self.cmp.do.insert_one({'name': 'bob', 'data': {'age': 0}})
        self.cmp.do.update_many({'name': 'bob'}, {'$setOnInsert': {'data.age': 1}}, True)
        self.cmp.compare.find()
        self.cmp.do.update_many({'name': 'bob'}, {'$setOnInsert': {'data.age1': 1}}, True)
        self.cmp.compare.find()
        self.cmp.do.update_many({'name': 'ann'}, {'$setOnInsert': {'data.age': 1}}, True)
        self.cmp.compare.find()
    ...
    def test__setOnInsert_subdocument_elemMatch(self):
        ...
        self.cmp.do.insert_one({'name': 'bob', 'data': [{'age': 0}, {'age': 1}]})
        self.cmp.do.update_many(
            {'name': 'bob', 'data': {'$elemMatch': {'age': 0}}}, {'$setOnInsert': {'data.$.age': 1}}
        )
        self.cmp.compare.find()
    ...
    def test__addToSet_nested(self):
        ...
        self.cmp.do.insert_one({'name': 'bob'})
        for _ in range(3):
            self.cmp.do.update_many({'name': 'bob'}, {'$addToSet': {'hat.color': 'green'}})
            self.cmp.compare.find({'name': 'bob'})
        for _ in range(3):
            self.cmp.do.update_many({'name': 'bob'}, {'$addToSet': {'hat.color': 'tall'}})
            self.cmp.compare.find({'name': 'bob'})
    ...
    def test__pull_query(self):
        ...
        self.cmp.do.update_many({'name': 'bob'}, {'$pull': {'hat': {'size': {'$gt': 6}}}})
        self.cmp.compare.find({'name': 'bob'})
        ...
        self.cmp.do.update_many({'name': 'bob'}, {'$pull': {'hat.sizes': {'size': {'$gt': 6}}}})
        self.cmp.compare.find({'name': 'bob'})
    ...
    def test__pull_nested_dict(self):
        ...
        self.cmp.do.insert_one(
            {
                ...
                'hat': [
                    {
                        'name': 'derby',
                        'sizes': [{'size': 'L', 'quantity': 3}, {'size': 'XL', 'quantity': 4}],
                        'colors': ['green', 'blue'],
                    },
                    {
                        'name': 'cap',
                        'sizes': [{'size': 'S', 'quantity': 10}, {'size': 'L', 'quantity': 5}],
                        'colors': ['blue'],
                    },
                ],
            }
        ...
        self.cmp.do.update_many(
            {'hat': {'$elemMatch': {'name': 'derby'}}}, {'$pull': {'hat.$.sizes': {'size': 'L'}}}
        )
        self.cmp.compare.find({'name': 'bob'})
    ...
    def test__pullAll(self):
        ...
        self.cmp.do.delete_many({})
        self.cmp.do.insert_one({'name': 'bob'})
        self.cmp.do.update_many({'name': 'bob'}, {'$pullAll': {'hat': ['green', 'blue']}})
        self.cmp.compare.find({'name': 'bob'})
    ...
    def test__push_dict(self):
        self.cmp.do.delete_many({})
        self.cmp.do.insert_one({'name': 'bob', 'hat': [{'name': 'derby', 'sizes': ['L', 'XL']}]})
        self.cmp.do.update_many(
    ...
    def test__push_each(self):
        self.cmp.do.delete_many({})
        self.cmp.do.insert_one({'name': 'bob', 'hat': ['green', 'tall']})
        self.cmp.do.update_many({'name': 'bob'}, {'$push': {'hat': {'$each': ['wide', 'blue']}}})
        self.cmp.compare.find({'name': 'bob'})
    ...
    def test__push_nested_dict_each(self):
        ...
        self.cmp.do.update_many(
            {'hat': {'$elemMatch': {'name': 'derby'}}},
            {
                '$push': {
                    'hat.$.sizes': {
                        '$each': [{'size': 'M', 'quantity': 6}, {'size': 'S', 'quantity': 1}]
                    }
                }
            },
        )
        ...
        self.cmp.do.insert_one(
            {
                'name': 'bob',
                'hat': [
                    {'name': 'derby', 'sizes': ['L', 'XL'], 'colors': ['green', 'blue']},
    ...
    def test__push_nested_attribute_each(self):
        self.cmp.do.delete_many({})
        self.cmp.do.insert_one({'name': 'bob', 'hat': {}})
        self.cmp.do.update_many({'name': 'bob'}, {'$push': {'hat.first': {'$each': ['a', 'b']}}})
        self.cmp.compare.find({'name': 'bob'})
    ...
    def test__rename_collection(self):
        ...
        # Exclude $clusterTime and operationTime because MongoDB replica set
        # returns real timestamps that mongomock cannot match
...
class MongoClientAggregateTest(_CollectionComparisonTest):
    ...
    def test__aggregate29(self):
        # group addToSet
        pipeline = [{'$group': {'_id': '$a', 'nb': {'$addToSet': '$count'}}}, {'$sort': {'_id': 1}}]
    ...
    def test__aggregate31(self):
        ...
        result = list(aggregations['fake'])
        self.assertEqual(len(result), len(expected))
        set_expected = {tuple(sorted(e.items())) for elt in expected for e in elt['set']}
        set_result = {tuple(sorted(e.items())) for elt in result for e in elt['set']}
            ...
            [
                {
                    '$addFields': {
                        'result': {
                            '$regexMatch': {'input': '$description', 'regex': Regex('line', 'i')}
                        },
                    }
                }
            ]
            ...
            [
                {
                    '$addFields': {
                        'result': {
                            '$regexMatch': {'input': '$description', 'regex': 'line', 'other': True}
                        },
                    }
                }
            ]
            ...
            [
                {
                    '$addFields': {
                        'result': {
                            '$regexMatch': {'input': '$description', 'regex': Regex('line', 'u')}
                        },
                    }
                }
            ]
    ...
    def test__aggregate_lookup_with_let(self):
        self.cmp.do.delete_many({})
        self.cmp.do.getattr('warehouses').insert_many(
            [
    ...
    def test__aggregate_if_null_multi_field(self):
        ...
        if version.parse('4.4') < SERVER_VERSION:
    ...
    def test_aggregate_bug_607(self):
        ...
        self.cmp.compare.aggregate(
            [{'$project': {'values_index': {'$arrayElemAt': ['$values', {'$add': [1, 1]}]}}}]
        )
    ...
    def test__aggregate_slice(self):
        ...
        self.cmp.compare.aggregate([{'$project': {'slice': {'$slice': ['$items', 0, 1]}}}])
        self.cmp.compare.aggregate([{'$project': {'slice': {'$slice': ['$items', 0, 5]}}}])
        self.cmp.compare.aggregate([{'$project': {'slice': {'$slice': ['$items', 5, 1]}}}])
        self.cmp.compare.aggregate([{'$project': {'slice': {'$slice': ['$items', 5, 5]}}}])
        self.cmp.compare.aggregate([{'$project': {'slice': {'$slice': ['$items', 0, 10000]}}}])
        self.cmp.compare.aggregate([{'$project': {'slice': {'$slice': ['$items', -5]}}}])
        self.cmp.compare.aggregate([{'$project': {'slice': {'$slice': ['$items', -10]}}}])
        self.cmp.compare.aggregate([{'$project': {'slice': {'$slice': ['$items', -5, 5]}}}])
        ...
        self.cmp.do.insert_one(
            {
                ...
                'dateTz': datetime.datetime(2000, 1, 1, 12, 30, 30, 12745, tzinfo=UTCPlus2()),
            }
        )
        self.cmp.compare.find_one()
        ...
        pipeline = [
            {
                '$addFields': {
                    ...
                    'dayOfWeek': {
                        '$dayOfWeek': {'date': '$start_date', 'timezone': 'America/New_York'}
                    },
                }
            },
            {'$project': {'_id': 0}},
    ...
    def test__aggregate_add_fields_with_max_min(self):
        ...
        pipeline = [
            {'$addFields': {'max_date': {'$max': '$dates'}, 'min_date': {'$min': '$dates'}}}
        ]
        self.cmp.compare.aggregate(pipeline)
    ...
    def test__aggregate_add_fields_with_sum_avg(self):
        self.cmp.do.delete_many({})
        self.cmp.do.insert_many([{'_id': 4, 'values': [10, 5, 7]}, {'_id': 5, 'values': []}])
        pipeline = [
            {'$addFields': {'max_val': {'$sum': '$values'}, 'min_val': {'$avg': '$values'}}}
        ]
        self.cmp.compare.aggregate(pipeline)
    ...
    def test_aggregate_project_with_boolean(self):
        ...
        self.cmp.compare.aggregate([{'$project': {'_id': 0, 'items': {'$and': [True]}}}])
        ...
        self.cmp.compare.aggregate([{'$project': {'_id': 0, 'items': {'$and': [True, True]}}}])

        self.cmp.compare.aggregate([{'$project': {'_id': 0, 'items': {'$and': [False, True]}}}])

        self.cmp.compare.aggregate([{'$project': {'_id': 0, 'items': {'$and': [True, False]}}}])

        self.cmp.compare.aggregate([{'$project': {'_id': 0, 'items': {'$and': [False, False]}}}])

        self.cmp.compare.aggregate([{'$project': {'_id': 0, 'items': {'$or': [True, True]}}}])

        self.cmp.compare.aggregate([{'$project': {'_id': 0, 'items': {'$or': [False, True]}}}])

        self.cmp.compare.aggregate([{'$project': {'_id': 0, 'items': {'$or': [True, False]}}}])

        self.cmp.compare.aggregate([{'$project': {'_id': 0, 'items': {'$or': [False, False]}}}])
    ...
    def test__aggregate_merge_objects(self):
        ...
        self.cmp.do.insert_many(
            [
                ...
                {'_id': ObjectId(), 'a': 2, 'b': {'c': '7', 'd': None, 'e': 9, 'f': '10'}},
...
def _COUNT_EXCEPTION_TYPE(cursor):
    ...
    raise AssertionError('Count should have failed')
